...
<body>
  ...
  <div class="grid">
    ...
    <div class="card">
      <h2>Status</h2>
      <div class="row"><span class="label">State</span><span class="value" id="status">-</span></div>
    ...
    <div class="card">
      ...
      <div class="btn-row">
        ...
        <button class="secondary" onclick="controlRobot('stop')">Stop</button>
        <button class="secondary" onclick="controlRobot('turn')">Turn</button>
      </div>
    ...
    <div class="card">
      ...
      <svg viewBox="0 0 200 200" width="180" height="180" style="display:block;margin:0 auto;">
        <defs>
          <clipPath id="horizonClip"><circle cx="100" cy="100" r="94"/></clipPath>
        </defs>
        <g clip-path="url(#horizonClip)">
          <g id="horizonGroup">
            <rect x="-300" y="-300" width="600" height="300" fill="#1f6feb" opacity="0.35"/>
            <rect x="-300" y="0" width="600" height="300" fill="#3fb950" opacity="0.25"/>
            <line x1="-300" y1="0" x2="300" y2="0" stroke="#e6edf3" stroke-width="2"/>
          </g>
        </g>
        <polygon points="100,84 91,106 109,106" fill="#f85149"/>
        <circle cx="100" cy="100" r="94" fill="none" stroke="var(--panel-border)" stroke-width="3"/>
      </svg>
      <div class="row"><span class="label">Tilt</span><span class="value" id="attitudeAngle">-</span></div>
    </div>
  ...
  <script>
    ...
    function refreshStatus() {
      get("/status").then(function(data) {
        ...
        document.getElementById("attitudeAngle").innerHTML = data.angle.toFixed(1) + "°";
        document.getElementById("horizonGroup").setAttribute(
          "transform", "rotate(" + (-data.angle) + " 100 100)"
        );
